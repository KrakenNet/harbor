...
"""harbor.nodes.nautilus.broker_node -- :class:`BrokerNode` built-in (FR-44, FR-46, design §8.1).
...
4. Calls :meth:`Broker.arequest` and patches the response onto state at
   ...
   §8.1 so downstream :mod:`harbor.fathom` mirroring can pick it up.
...
class BrokerNode(NodeBase):
    ...
    Harbor's POV (design §8.1). Replay-safe by default.
    ...
    def _envelope(response: Any) -> dict[str, Any]:
        ...
        ``__harbor_provenance__`` key carrying the design §8.1 fields:
